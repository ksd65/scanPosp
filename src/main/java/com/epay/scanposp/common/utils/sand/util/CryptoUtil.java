...
public abstract class CryptoUtil
{
    ...
    throws Exception
  {
    try
    {
      ...
      throw new Exception(String.format("数字签名时没有[%s]此类算法", new Object[] { signAlgorithm }));
    ...
    throws Exception
  {
    ...
    try {
      ...
      throw new Exception(String.format("验证数字签名时没有[%s]此类算法", new Object[] { signAlgorithm }), e);
    } catch (InvalidKeyException e) {
  ...
  public static boolean verifyDigitalSign(byte[] plainBytes, byte[] signBytes, X509Certificate cert, String signAlgorithm)
    throws Exception
  {
    boolean isValid = false;
    try {
      ...
      throw new Exception(String.format("验证数字签名时没有[%s]此类算法", new Object[] { signAlgorithm }));
    } catch (InvalidKeyException e) {
      throw new Exception("验证数字签名时公钥无效", e);
    } catch (SignatureException e) {
      throw new Exception("验证数字签名时出现异常", e);
    ...
    throws Exception
  {
    ...
    try
    {
      ...
      throw new Exception(String.format("没有[%s]此类加密算法", new Object[] { cipherAlgorithm }));
    } catch (NoSuchPaddingException e) {
      throw new Exception(String.format("没有[%s]此类填充模式", new Object[] { cipherAlgorithm }));
    } catch (InvalidKeyException e) {
    ...
    throws Exception
  {
    ...
    try
    {
      ...
      throw new Exception(String.format("没有[%s]此类解密算法", new Object[] { cipherAlgorithm }));
    } catch (NoSuchPaddingException e) {
      throw new Exception(String.format("没有[%s]此类填充模式", new Object[] { cipherAlgorithm }));
    } catch (InvalidKeyException e) {
      throw new Exception("无效密钥", e);
