...
import com.epay.scanposp.common.constant.KQConfig;
...
import com.epay.scanposp.common.constant.ZNYConfig;
...
public class CashierDeskController {
	...
	public JSONObject qrcodePay(HttpServletRequest request,HttpServletResponse response){
		...
		logger.info("扫码支付下游入参[{}]",  JSONObject.fromObject(inparam).toString() );
...
//			respJSONObject.put("resEntity", resEntityObject);
	...
	public JSONObject validMemberInfoForQrcode(String memberCode,String orderNum,String payMoney,String platformType,String payType,String signOrginalStr,String signedStr,String callbackUrl,String clientIp,String ipReal,String userAgent){
		...
		if(RouteCodeConstant.ESK_ROUTE_CODE.equals(routeCode)){
			result = eskScanQrcodePay(platformType,payType,memberInfo,memberPayType, payMoney, orderNum, callbackUrl , merchantCode ,routeCode,aisleType,clientIp);
		}else if(RouteCodeConstant.FT_ROUTE_CODE.equals(routeCode)){
			result = ftScanQrcodePay(platformType,payType,memberInfo, memberPayType,payMoney, orderNum, callbackUrl , merchantCode ,aisleType,clientIp);
		}else if(RouteCodeConstant.YL_ROUTE_CODE.equals(routeCode)){
			result = ylScanQrcodePay(platformType,payType,memberInfo,memberPayType, payMoney, orderNum, callbackUrl , merchantCode ,aisleType);
			...
			result = hlbScanQrcodePay(platformType,payType,memberInfo, memberPayType,payMoney, orderNum, callbackUrl , merchantCode ,routeCode,aisleType,clientIp);
		...
		}else if(RouteCodeConstant.KQ_ROUTE_CODE.equals(routeCode)){
			memberInfo.setSettleType("0");
			result = kqScanQrcodePay(platformType,payType,memberInfo,memberPayType, payMoney, orderNum, callbackUrl , merchantCode ,routeCode,aisleType,clientIp,userAgent);
		}else{
...
//			resData.put("orderCode", orderCode);     //暂不回传我方的订单编码
	 ...
	 * @return
	 */
	public JSONObject eskScanQrcodePay(String platformType,String payType,MemberInfo memberInfo,MemberPayType memberPayType,String payMoney,String orderNumOuter,String callbackUrl,MemberMerchantCode merchantCode,String routeCode,String aisleType,String ip) {
		JSONObject result = new JSONObject();
		try {
			// 插入一条收款记录
			String orderCode = CommonUtil.getOrderCode();
	 ...
	 * @return
	 */
	public JSONObject ftScanQrcodePay(String platformType,String payType,MemberInfo memberInfo,MemberPayType memberPayType, String payMoney,String orderNumOuter,String callbackUrl,MemberMerchantCode merchantCode,String aisleType,String clientIp) {
		JSONObject result = new JSONObject();
		try {
			// 插入一条收款记录
			String orderCode = CommonUtil.getOrderCode();
...
//					wxjsapiStr.put("package_str", wxjsapiStr.getString("package"));
	...
	public JSONObject orderQuery(HttpServletRequest request,HttpServletResponse response) {
		...
		try {
			...
			logger.info("订单查询下游入参[{}]",  JSONObject.fromObject(inparam).toString() );
			String orderNum = request.getParameter("orderNum");
			String memberCode = request.getParameter("memberCode");
			String signStr = request.getParameter("signStr");
			...
			if(RouteCodeConstant.ESK_ROUTE_CODE.equals(routeCode)||RouteCodeConstant.ESKXF_ROUTE_CODE.equals(routeCode)||RouteCodeConstant.ESKHLB_ROUTE_CODE.equals(routeCode)||RouteCodeConstant.ESKKJ_ROUTE_CODE.equals(routeCode)||RouteCodeConstant.ESKWG_ROUTE_CODE.equals(routeCode)||RouteCodeConstant.ESK13_ROUTE_CODE.equals(routeCode)||RouteCodeConstant.ESKWGD0_ROUTE_CODE.equals(routeCode)){
				// 调用支付通道
				String serverUrl = ESKConfig.msServerUrl;
			//	PrivateKey hzfPriKey = CryptoUtil.getRSAPrivateKey();
				String tranCode = "004";
				...
				reqData.put("tranCode", tranCode);
				if(RouteCodeConstant.ESKKJ_ROUTE_CODE.equals(routeCode)||RouteCodeConstant.ESKWG_ROUTE_CODE.equals(routeCode)||RouteCodeConstant.ESKWGD0_ROUTE_CODE.equals(routeCode)){
		    ...
		    }else if(RouteCodeConstant.HX_ROUTE_CODE.equals(routeCode)||RouteCodeConstant.GRSM_ROUTE_CODE.equals(routeCode)||RouteCodeConstant.TLWD_ROUTE_CODE.equals(routeCode)||RouteCodeConstant.TLKJ_ROUTE_CODE.equals(routeCode)||RouteCodeConstant.TLYL_ROUTE_CODE.equals(routeCode)){
				...
				if ("000000".equals(returnCode)) {
					...
					if(!sign.equals(signed)){
						...
						if("0".equals(resultcode)){
							String status = respJSONObject.getString("TRADE_STATE");
							if("SUCCESS".equals(status)){
			        			result.put("oriRespType", "S");
								result.put("oriRespCode", "000000");
								result.put("oriRespMsg", "支付成功");
								result.put("totalAmount", respJSONObject.getString("AMOUNT"));
							...
							resultMsg = respJSONObject.getString("ERR_MSG");
							result.put("returnCode", "0012");
							result.put("returnMsg", resultMsg);
						}
					}
		        ...
		        String serverUrl = ZNYConfig.msServerUrl+"/select";
				
				JSONObject reqData = new JSONObject();
				reqData.put("orderuid", debitNote.getRemarks());
				reqData.put("uid", debitNote.getMerchantCode());
				reqData.put("orderid", debitNote.getOrderCode());
				reqData.put("price", debitNote.getMoney().toString());
				
				String srcStr1 = reqData.getString("uid")+reqData.getString("orderid")+reqData.getString("price")+reqData.getString("orderuid")+merchantKey.getPrivateKey();
				logger.info("Sign Before MD5: {}", srcStr1);
				String sign = MD5Util.MD5Encode(srcStr1);
				reqData.put("key", sign);
				
				logger.info("智能云订单查询请求数据[{}]", new Object[] { reqData.toString() });
				...
				logger.info("智能云订单查询返回报文[{}]", new Object[] { respStr });
				...
				if ("200".equals(returnCode)) {
					...
					if("1".equals(status)||"2".equals(status)){
						...
						result.put("oriRespCode", "000000");
						result.put("oriRespMsg", "支付成功");
						result.put("totalAmount", jsonObject.getString("realprice"));
					...
					resultMsg = URLDecoder.decode(jsonObject.getString("msg"), "UTF-8");
					result.put("returnCode", "0012");
					result.put("returnMsg", resultMsg);
				}
		        ...
		        String srcStr1 = orderCode+debitNote.getOrderCode()+debitNote.getMerchantCode();
			...
			//	System.out.println("第二次加密结果："+sign);
				...
				param.put("USER_ID", debitNote.getMerchantCode());
				param.put("SIGN_TYPE", "03");
				param.put("SIGN", sign);
				param.put("ORG_ORDER_ID", debitNote.getOrderCode());
				logger.info("米联订单查询参数[{}]",JSONObject.fromObject(param).toString() );
	            ...
	            byte[] b = HttpClient4Util.getInstance().doPost(serverUrl, null, nvps);
				String respStr = new String(b, "UTF-8");
				logger.info("米联订单查询返回报文[{}]", new Object[] { respStr });
...
//			 	request2.setPublicKeyPath("D:/dev_pub.cer");
	 ...
	 * @return
	 */
	public JSONObject ylScanQrcodePay(String platformType,String payType,MemberInfo memberInfo,MemberPayType memberPayType,String payMoney,String orderNumOuter,String callbackUrl,MemberMerchantCode merchantCode,String aisleType) {
		JSONObject result = new JSONObject();
		JSONObject resData = new JSONObject();
		try {
			// 插入一条收款记录
	 ...
	 * @return
	 */
	public JSONObject hlbScanQrcodePay(String platformType,String payType,MemberInfo memberInfo,MemberPayType memberPayType,String payMoney,String orderNumOuter,String callbackUrl,MemberMerchantCode merchantCode,String routeCode,String aisleType,String ip) {
		JSONObject result = new JSONObject();
		try {
			String payTypeStr = "";
			String payMethod = PayTypeConstant.PAY_METHOD_SMZF;
	...
	//个人扫码支付
	public JSONObject grsmScanQrcodePay(String platformType,String payType,MemberInfo memberInfo,MemberPayType memberPayType,String payMoney,String orderNumOuter,String callbackUrl,MemberMerchantCode merchantCode,String routeCode,String aisleType,String ip) {
		JSONObject result = new JSONObject();
		try {
			String payTypeStr = "";
			String payMethod = PayTypeConstant.PAY_METHOD_SMZF;
	...
	public JSONObject tlScanQrcodePay(String platformType,String payType,MemberInfo memberInfo,MemberPayType memberPayType,String payMoney,String orderNumOuter,String callbackUrl,MemberMerchantCode merchantCode,String routeCode,String aisleType,String ip) {
		JSONObject result = new JSONObject();
		try {
			...
			MemberMerchantKeyExample memberMerchantKeyExample = new MemberMerchantKeyExample();
	        memberMerchantKeyExample.createCriteria().andRouteCodeEqualTo(RouteCodeConstant.TL_ROUTE_CODE).andMerchantCodeEqualTo(merCode).andDelFlagEqualTo("0");
	        List<MemberMerchantKey> keyList = memberMerchantKeyService.selectByExample(memberMerchantKeyExample);
	        if(keyList == null || keyList.size()!=1){
	            result.put("returnCode", "0008");
				result.put("returnMsg", "商户私钥未配置");
			...
			JSONObject reqData = new JSONObject();
			String eskPayType = "PAY_WECHAT_NATIVE";
			if("2".equals(payType)){
				eskPayType="PAY_ALIPAY_NATIVE";
			}else if("3".equals(payType)){
			...
			if("000000".equals(jsonObject.getString("ACTION_RETURN_CODE"))){
				...
				if(sign.equals(signed)){
					String resultCode = respJSONObject.getString("RESULT_CODE");
					if("0".equals(resultCode)){
						result.put("qrCode", SecurityUtil.base64Encode(respJSONObject.get("CODE_URL").toString()));
						result.put("returnCode", "0000");
						result.put("returnMsg", "成功");
						flag = true;
					}else{
	...
	@RequestMapping("/cashierDesk/tlPayNotify")
	public void tlPayNotify(HttpServletRequest request,HttpServletResponse response) {
		String respString = "SUCCESS";
		try {
			String responseStr = HttpUtil.getPostString(request);
			...
			logger.info("tlPayNotify回调通知报文[{}]",  responseStr );
			
			JSONObject obj = JSONObject.fromObject(responseStr);
			String sign = obj.getString("SIGN");
			JSONObject respObj = JSONObject.fromObject(obj.getString("ACTION_INFO"));
			
			
			String reqMsgId = respObj.getString("OUT_TRADE_NO");
        	TradeDetailExample tradeDetailExample = new TradeDetailExample();
        	tradeDetailExample.or().andOrderCodeEqualTo(reqMsgId);
        	List<TradeDetail> tradeDetailList = tradeDetailService.selectByExample(tradeDetailExample);
        	if(tradeDetailList!=null && tradeDetailList.size()>0){
            ...
            String result_code = respObj.getString("PAY_RESULT");
            String result_message = "";
            if(respObj.containsKey("PAY_INFO")){
            	result_message = respObj.getString("PAY_INFO");
            }
        	MsResultNoticeExample msResultNoticeExample = new MsResultNoticeExample();
			msResultNoticeExample.or().andOrderCodeEqualTo(reqMsgId);
			List<MsResultNotice> msResultNoticeList = msResultNoticeService.selectByExample(msResultNoticeExample);
			if(null == msResultNoticeList || msResultNoticeList.size() == 0){
				if (debitNote != null ) {
					...
					tradeDetail.setMemberId(debitNote.getMemberId());
					tradeDetail.setMerchantCode(debitNote.getMerchantCode());
					tradeDetail.setChannelNo(respObj.getString("TRANSACTION_ID"));
					
					tradeDetail.setMemberCode(debitNote.getMemberCode());
					tradeDetail.setMoney(new BigDecimal(respObj.getString("AMOUNT")));
					tradeDetail.setPayTime(DateUtil.getDateTimeStr(debitNote.getCreateDate()));
					tradeDetail.setPtSerialNo(debitNote.getOrderCode());
					tradeDetail.setOrderCode(debitNote.getOrderCode());
					tradeDetail.setReqDate(DateUtil.getDateTimeStr(debitNote.getCreateDate()));
					...
					MsResultNotice msResultNotice = new MsResultNotice();
					msResultNotice.setMoney(new BigDecimal(respObj.getString("AMOUNT")));
					msResultNotice.setOrderCode(reqMsgId);
					msResultNotice.setPtSerialNo(respObj.getString("TRANSACTION_ID"));
	...
	//通联扫码回调（新）
	@RequestMapping("/cashierDesk/tlWdPayNotify")
	public void tlWdPayNotify(HttpServletRequest request,HttpServletResponse response) {
	...
	public JSONObject znyScanQrcodePay(String platformType,String payType,MemberInfo memberInfo,MemberPayType memberPayType,String payMoney,String orderNumOuter,String callbackUrl,MemberMerchantCode merchantCode,String routeCode,String aisleType,String ip) {
		JSONObject result = new JSONObject();
		try {
			...
			debitNote.setStatus("0");
			debitNote.setIp(ip);
			debitNote.setRemarks(orderDesc);
			debitNote.setTxnMethod(payMethod);
			if("1".equals(payType)){
				debitNote.setTxnType("1");
				debitNote.setMemberCode(memberInfo.getWxMemberCode());
			...
			String callBack = SysConfig.serverUrl + "/cashierDesk/znyPayNotify";
			
			// 调用支付通道
			String eskPayType = "20001";
			if("2".equals(payType)){
				eskPayType="10001";
			}
			
			String serverUrl = ZNYConfig.msServerUrl+"/action";
			
			JSONObject reqData = new JSONObject();
			reqData.put("uid", merCode);
			reqData.put("orderid", orderCode);
			reqData.put("istype", eskPayType);
			reqData.put("price", payMoney);
			reqData.put("goodsname", orderDesc);
			reqData.put("orderuid", orderDesc);
			reqData.put("notify_url", callBack);
			reqData.put("return_url", SysConfig.frontUrl);
			
			String srcStr = reqData.getString("goodsname")+reqData.getString("istype")
					+reqData.getString("notify_url")+reqData.getString("orderid")
					+reqData.getString("orderuid")+reqData.getString("price")
					+reqData.getString("return_url")+merchantKey.getPrivateKey()+reqData.getString("uid");
			
			logger.info("Sign Before MD5: {}", srcStr);
			String sign = MD5Util.MD5Encode(srcStr);
			reqData.put("key", sign);
			
			logger.info("智能云扫码请求数据[{}]", new Object[] { reqData.toString() });
			...
			byte[] b = HttpClient4Util.getInstance().doPost(serverUrl, null, nvps);
			String respStr = new String(b, "UTF-8");
			logger.info("智能云扫码返回报文[{}]", new Object[] { respStr });
			...
			if("200".equals(result_code)){
				...
				result.put("qrCode", SecurityUtil.base64Encode(dataObj.get("qrcode").toString()));
				result.put("returnCode", "0000");
				result.put("returnMsg", "成功");
				flag = true;
			}else{
				result_message = URLDecoder.decode(jsonObject.getString("msg"), "UTF-8");
			}
	...
	@RequestMapping("/cashierDesk/znyPayNotify")
	public void znyPayNotify(HttpServletRequest request,HttpServletResponse response) {
		...
		//	String responseStr = "uid=57752125&ordno=A075133792402286&orderid=20180501105833992524&price=1.00&realprice=1.00&orderuid=%E7%A6%8F%E5%B7%9E%E5%8F%B0%E6%B1%9F%E4%B8%87%E8%BE%BE%E5%B0%8F%E5%90%83%E5%BA%97%E6%94%B6%E6%AC%BE&key=cabaacf8ac518c51f04f4367b7b9df01";
			...
			for(int i=0;i<arr.length;i++){
				...
				String[] arr1 = str.split("=");
				if(arr1.length==2){
					respObj.put(arr1[0], URLDecoder.decode(arr1[1], "UTF-8"));
				}
			}
			//JSONObject respObj = JSONObject.fromObject(responseStr);
			String sign = respObj.get("key");
			respObj.remove("key");
            ...
            String srcStr = respObj.get("orderid")+respObj.get("orderuid")+respObj.get("ordno")+respObj.get("price")+respObj.get("realprice")+
            		keyList.get(0).getPrivateKey();
			logger.info("Sign Before MD5: {}", srcStr);
			String mySign  = MD5Util.MD5Encode(srcStr);
			logger.info("Sign After MD5: {}", mySign);
            if (!sign.equals(mySign)){
				respString = "fail";
                logger.info("验证签名不通过");
                response.getWriter().write(respString);
            ...
            String result_code = "00";//没有返回编码，写死
            String result_message = "";
            if(respObj.containsKey("refMsg")){
            	result_message = URLDecoder.decode(respObj.get("refMsg"), "UTF-8");
            }
        	MsResultNoticeExample msResultNoticeExample = new MsResultNoticeExample();
			msResultNoticeExample.or().andOrderCodeEqualTo(reqMsgId);
			List<MsResultNotice> msResultNoticeList = msResultNoticeService.selectByExample(msResultNoticeExample);
			if(null == msResultNoticeList || msResultNoticeList.size() == 0){
				if (debitNote != null ) {
					...
					tradeDetail.setMemberId(debitNote.getMemberId());
					tradeDetail.setMerchantCode(debitNote.getMerchantCode());
					tradeDetail.setChannelNo(respObj.get("ordno"));
					
					tradeDetail.setMemberCode(debitNote.getMemberCode());
					tradeDetail.setMoney(new BigDecimal(respObj.get("realprice")));
					tradeDetail.setPayTime(DateUtil.getDateTimeStr(debitNote.getCreateDate()));
					tradeDetail.setPtSerialNo(debitNote.getOrderCode());
					tradeDetail.setOrderCode(debitNote.getOrderCode());
					tradeDetail.setReqDate(DateUtil.getDateTimeStr(debitNote.getCreateDate()));
					...
					MsResultNotice msResultNotice = new MsResultNotice();
					msResultNotice.setMoney(new BigDecimal(respObj.get("realprice")));
					msResultNotice.setOrderCode(reqMsgId);
					msResultNotice.setPtSerialNo(respObj.get("ordno"));
	...
	public JSONObject kqScanQrcodePay(String platformType,String payType,MemberInfo memberInfo,MemberPayType memberPayType,String payMoney,String orderNumOuter,String callbackUrl,MemberMerchantCode merchantCode,String routeCode,String aisleType,String ip,String userAgent) {
		JSONObject result = new JSONObject();
		try {
			String payTypeStr = "";
			String payMethod = PayTypeConstant.PAY_METHOD_SMZF;
			...
			JSONObject ipResult = commonUtilService.checkLimitIp(PayTypeConstant.PAY_METHOD_SMZF, payTypeStr, memberInfo.getId(), routeCode, merCode, ip);
			if(null != ipResult){
				debitNote.setStatus("6");
				debitNoteService.insertSelective(debitNote);
				return ipResult;
			...
			String callBack = SysConfig.serverUrl + "/cashierDesk/kqPayNotify";
			String serverUrl = KQConfig.payServerUrl;
			String charset = "utf-8";
			
			Map<String,String> sPara = new HashMap<String,String>();
			String goodsName = memberInfo.getName() + "收款";
			sPara.put("requestTime",new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()));
			sPara.put("externalTraceNo",orderCode);
			sPara.put("merchantCode",merCode);
			sPara.put("merchantId",merchantKey.getAppId());
			sPara.put("terminalId",merchantKey.getPrivateKeyPassword());
			sPara.put("amt",payMoney);
			sPara.put("returnUrl",SysConfig.frontUrl);
			sPara.put("productInfo",goodsName);
			
			String srcSign= StringUtil.orderedKey(sPara)+"&key="+merchantKey.getPrivateKey();
			System.out.println("srcSign===="+srcSign);
			
			
			
			String sign = DigestUtils.md5Hex(srcSign).toUpperCase();
			sPara.put("sign",sign);
			
			logger.info("快钱扫码请求数据[{}]", new Object[] { JSONObject.fromObject(sPara).toString() });
			
			serverUrl = serverUrl+ "?requestTime=" + sPara.get("requestTime") + "&externalTraceNo=" + sPara.get("externalTraceNo")
					+ "&merchantCode=" + sPara.get("merchantCode") + "&merchantId=" + sPara.get("merchantId")
					+ "&terminalId=" + sPara.get("terminalId") + "&productInfo=" + sPara.get("productInfo")
					+ "&amt=" + sPara.get("amt") + "&returnUrl=" + sPara.get("returnUrl")
					+ "&secretInfo=" + sPara.get("sign");
